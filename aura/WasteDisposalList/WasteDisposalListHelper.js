({
	getData : function(component, event, refresh) {
        ...
        var params = { tmId: tmId, category: 'Waste Disposal' };
        this.callServerMethod(component, event, "c.GetTMLinesByCategory", params, function(data){
            for (var i = 0; i < data.length; i++) {                
                ...
                if (data[i].Facility__r) {
                    data[i].FacilityName = data[i].Facility__r.Name;
                }
            }
    ...
    confirmCopyManifest : function(component, event) {
        //component.find("modal").open('T&M', 'Copy Manifest', 'Are you sure you want to copy manifest information from this line to all the lines below it?', 'confirmation', component, event, this.copyManifestCallback.bind(this));
        var buttons = [];
		buttons.push({ "label": 'No', "variant": 'neutral', "action": { "callback": this.cancelCallback.bind(this, component, event) }});        
        buttons.push({ "label": 'Yes', "variant": 'brand', "action": { "callback": this.copyManifestCallback.bind(this, component, event) }});
        this.openModal(component, event, 'Copy Manifest', 'Are you sure you want to copy manifest information from this line to all the lines below it?', buttons, null, null, null);
    },
    ...
    copyManifest : function(component, event) {
        ...
        for (var i = lineIndex + 1; i < data.length; i++) {
            data[i].BOL_Manifest__c = data[i-1].BOL_Manifest__c;
        }
        component.set("v.data", data);
        this.showToast(component, 'success', 'T&M', 'Manifest information copied!', 'dismissable');
    },
