({
    search : function(component, event, keyword, callback) {
        ...
        if (useStaticOption) {
            ...
            var recrd = component.get("v.name"); 
            //alert('---'+JSON.stringify(recrd));
             //alert('-1--'+recrd.Bill_to_Customer_No__r.Sage_Customer_No__c);
            //alert('-2--'+recrd.Service_Center__r.Name);
            
            var params = {
                "d365CustomerId":recrd.Bill_to_Customer_No__r.Sage_Customer_No__c,
                "serviceCenterId":recrd.Service_Center__r.Id,
                "keyword": keyword
            };
            component.set("v.showLookupSpinner", true);
            this.callServerMethod(component, event, "c.fetchRecords", params, function (records) {
                //$A.util.addClass(spinner, "slds-hide");
                component.set("v.result", records.projectIds);
        ...
        if (!dropdownbox) {
            ...
            $A.createComponent(
                "c:LookupInputDropdownApproval",
                {
                    ...
                    "message":component.getReference("v.message")
                },
