({
    search : function(component, event, keyword, callback) {
        ...
        if (useStaticOption) {
            ...
            var facilityId =  component.get("v.facility");
            var recrd = component.get("v.name");
            var recId = null;
            var genId = null;
            var scId = null;
            if(sObjectName == 'Sales_Line__c')
            {
                recId = recrd.Id; 
                genId = recrd.EqaiGeneratorId__c;
                scId = recrd.Service_Center__c;
            }           	
            else
            {
               recId =  component.get("v.name");
            }
           
            var params = {
                "facility": sObjectName,
                "keyword": keyword,
                "tmId":recId,
            ...
            this.callServerMethod(component, event, "c.fetchRecords", params, function (records) {
                ...
                component.set("v.result", records.approvals);
