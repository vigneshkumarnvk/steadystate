({
    ...
    getWorksheet: function (component, event, worksheetIndex) {
        ...
        this.callServerMethod(component, event, "c.getWorksheet", params, function (response) {
            ...
            var MAX_WORKSHEET_LINES_EDIT_COUNT = component.get("v.MAX_WORKSHEET_LINES_EDIT_COUNT");
            if (newWorksheet.WorksheetLines.length > MAX_WORKSHEET_LINES_EDIT_COUNT) {
                newWorksheet.WorksheetLines = [];
                this.showToast(component, "", "This job task has too many T&Ms, please choose T&Ms to add to the worksheet.", "info", "dismissible");
            }
            else {
            ...
            activeWorksheets = this.sortActiveWorksheetTabs(activeWorksheets);
            component.set("v.activeWorksheets", activeWorksheets);
            //ticket 20170 >>

            //TM lookup <<
            this.createTMLookupOptions(component, event);
            //TM lookup >>
            ...
            */
            var activeWorksheetIndex = this.getWorksheetIndex(newWorksheet, activeWorksheets);
            //ticket 20170 >>
            component.set("v.selectedTabId", 'tab' + activeWorksheetIndex); //activate the tab
        });
    },
    getWorksheetLines : function(component, event, worksheetIndex, salesOrderJobTaskId, tmId) {
        var salesOrderId = component.get("v.salesOrderId");
        var params = { "salesOrderId": salesOrderId, "salesOrderJobTaskId": salesOrderJobTaskId, "tmId": tmId };
        this.callServerMethod(component, event, "c.getWorksheetLines", params, function(response) {
            ...
            */
            ...
            if (activeWorksheetIndex == -1) {
                ...
                component.set("v.worksheets[" + worksheetIndex + "].Selected", true);
                //ticket 20170 <<
                ...
                var activeWorksheet = activeWorksheets[activeWorksheetIndex];
                var MAX_WORKSHEET_LINES_EDIT_COUNT = component.get("v.MAX_WORKSHEET_LINES_EDIT_COUNT");
                if (activeWorksheet.WorksheetLines.length == 0 || activeWorksheet.WorksheetLines.length + newWorksheet.WorksheetLines.length <= MAX_WORKSHEET_LINES_EDIT_COUNT) {
                    ...
                    component.set("v.worksheets[" + worksheetIndex + "]", worksheet);
                    this.showToast(component, "", "There are too many lines in the worksheet. You cannot add more T&Ms.", "warning", "dismissible");
                    return;
                }
            }
            ...
            if (jobTaskLine) {
                if (Array.isArray(jobTaskLine)) {
                    if (jobTaskLine[activeWorksheetIndex]) {
                        jobTaskLine[activeWorksheetIndex].closeInlineEdit();
                    }
                } else {
                    if (jobTaskLine) {
                        jobTaskLine.closeInlineEdit();
                    }
                }
            }
    ...
    validateWorksheets: function (component, event, invoice, resolve, reject) {
        ...
        //ticket 20170 <<
        /*
        var ok = worksheetTabs.reduce(function (valid, worksheetTab) {
        ...
        */
        var activeWorksheets = component.get("v.activeWorksheets");
        var ok = true;
        for (var i = 0; i < worksheetTabs.length; i++) {
            var worksheetTab = worksheetTabs[i];
            var result = worksheetTab.validateLines(invoice);
            //ticket 20797 <<
            /*
            //ticket 20398 <<
            //activeWorksheets[i].error = !result;
            activeWorksheets[i] = worksheetTab.get("v.worksheet");
            activeWorksheets[i].error = !result;
            component.set("v.activeWorksheets[" + i + "]", activeWorksheets[i]);
            //ticket 20398 >>
            */
            var activeWorksheet = worksheetTab.get("v.worksheet");
            var worksheetIndex = parseInt(worksheetTab.get("v.worksheetIndex"));
            activeWorksheets[worksheetIndex].error = !result;
            component.set("v.activeWorksheets[" + worksheetIndex + "]", activeWorksheets[worksheetIndex]);
            //ticket 20797 >>
    ...
    confirmCreateInvoice: function (component, event, activeWorksheets) {
        ...
        if (lineCount == 0) {
            if(salesOrder.Service_Center__r.Include_SO_in_EQAI_Invoice_Integration__c == true)
            	this.showToast(component, "Error", "You have not select lines to Billing Package.", "error", "dismissible");
            else
        ...
        /*
         *   Ticket#21357 add sales order bill-to contact checking.
         */
       
        if(salesOrder.Bill_to_Contact__r && salesOrder.Bill_to_Contact__r.Blocked__c === true){
            this.showToast(component, "Error", "Invoice cannot be created Sales Order Bill-to Contact has been blocked/inactive.  Please update Sales Order Bill-to Contact before continuing.", "error", "dismissible");
            return;
        }
        /*
        if (amount == 0) {
    ...
    promptWizard : function(component, event, activeWorksheets, resolve, reject) {
        ...
        for (var i = 0; i < activeWorksheets.length; i++) {
            ...
            for (var j = 0; j < activeWorksheet.WorksheetLines.length; j++) {
                var worksheetLine = activeWorksheet.WorksheetLines[j];
                if (!worksheetLine.Parent_Line__r && worksheetLine.Wizard_Question_Answered__c != true) {
                    lineCount++;
                }
            }
        }

        //console.log("wizard questions lines " + lineCount);
        if (lineCount == 0) {
            resolve();
            return;
        }

        //console.time("prepareWizardQuestions");
        var salesOrder = component.get("v.salesOrder");
        ...
        this.callServerMethod(component, event, "c.prepareWizardQuestions", params, function (response) {
            //console.timeEnd("prepareWizardQuestions")
    ...
    */
    ...
    saveWorksheets: function (component, event, activeWorksheets, redirect, resolve) {
        //console.log("save worksheets")
        var salesOrderId = component.get("v.salesOrderId");
        var worksheets = component.get("v.worksheets");
        var mapWorksheetsBySalesOrderJobTaskId = new Map();
        for (var i = 0; i < worksheets.length; i++) {
            var worksheet = worksheets[i];
            mapWorksheetsBySalesOrderJobTaskId.set(worksheet.SalesOrderJobTask.Id, worksheet);
        }
        for (var i = 0; i < activeWorksheets.length; i++) {
            var activeWorksheet = activeWorksheets[i];
            activeWorksheet.RelatedInfos = [];
            var worksheet = mapWorksheetsBySalesOrderJobTaskId.get(activeWorksheet.SalesOrderJobTask.Id);
            for (var j = 0; j < worksheet.RelatedInfos.length; j++) {
                var relatedInfo = worksheet.RelatedInfos[j];
                if (relatedInfo.TM.Selected == true) {
                    activeWorksheet.RelatedInfos.push(JSON.parse(JSON.stringify(relatedInfo)));
                }
            }
        }

        //console.time('saveWorksheets');
        var params = {"salesOrderId": salesOrderId, "JSONWorksheets": JSON.stringify(activeWorksheets)};
        this.callServerMethod(component, event, "c.saveWorksheets", params, function(response) {
                //console.timeEnd('saveWorksheets');
                if (redirect == true) {
                    this.navigateToSObject(component, event, salesOrderId);
                } else {
            ...
            function(error) {
                if (error.includes('UID__c duplicates value on record with id')) {
                    this.showToast(component, "", "Another user has just saved the worksheet at the same time you attempted to save your changes. Please try to save again. If you continue to get this error, please contact system administrator.", "error", "sticky");
                } else { //Ticket#20245
                    this.showToast(component, "dismissible", error, "error", "dismissible");
                }
            }
        );
    },
    ...
    confirmDeselectWorksheet: function(component, event, worksheetIndex) {
        var buttons = [];
        buttons.push({ "label": 'Cancel', "variant": 'neutral', "action": { "callback": this.cancelDeselectWorksheetCallback.bind(this, component, event, worksheetIndex) }});
        buttons.push({ "label": 'OK', "variant": 'brand', "action": { "callback": this.confirmDeselectWorksheetCallback.bind(this, component, event, worksheetIndex) }});
    ...
    confirmDeselectWorksheetCallback : function(component, event, worksheetIndex) {
        this.closeModal(component, event);
        var worksheets = component.get("v.worksheets");
        var worksheet = worksheets[worksheetIndex];
        var activeWorksheets = component.get("v.activeWorksheets");
    ...
    cancelDeselectWorksheetCallback : function(component, event, worksheetIndex) {
        this.closeModal(component, event);
        var worksheets = component.get("v.worksheets");
        worksheets[worksheetIndex].Selected = true;
        component.set("v.worksheets[" + worksheetIndex + "]", worksheets[worksheetIndex]);
    },
    confirmDeselectTM: function(component, event, worksheetIndex, salesOrderJobTaskId, tmId) {
        //check bundle status, disallowed if the T&M contains bundle line that bundles other TMs' lines
        var worksheets = component.get("v.worksheets");
        var worksheet = worksheets[worksheetIndex];
        var activeWorksheets = component.get("v.activeWorksheets");
        var ok = true;
                ...
                if (bundleLineNos.length > 0) {
                    for (var j = 0; j < lineNos.length; j++) {
                        if (bundleLineNos.includes(lineNos[j])) {
                            ...
                            this.showToast(component, "", "There are other T&Ms' lines bundled by this T&M. You cannot remove the T&M from the worksheet.", "error", "dismissible")
                            ok = false;
                        }
                    }
                }
        ...
        if (ok) {
            var buttons = [];
            buttons.push({ "label": 'Cancel', "variant": 'neutral', "action": { "callback": this.cancelDeselectTMCallback.bind(this, component, event, worksheetIndex, salesOrderJobTaskId, tmId) }});
            buttons.push({ "label": 'OK', "variant": 'brand', "action": { "callback": this.confirmDeselectTMCallback.bind(this, component, event, worksheetIndex, salesOrderJobTaskId, tmId) }});
            this.openModal(component, event, 'Deselect T&M', 'Are you sure you want to take out T&M lines from the worksheet?', buttons, null, null, null);
        }
    },
    confirmDeselectTMCallback : function(component, event, worksheetIndex, salesOrderJobTaskId, tmId) {
        this.closeModal(component, event);
        var worksheets = component.get("v.worksheets");
        var worksheet = worksheets[worksheetIndex];
        var activeWorksheets = component.get("v.activeWorksheets");
        ...
        for (var i = 0; i < activeWorksheets.length; i++) {
            ...
            if (activeWorksheet.SalesOrderJobTask.Id == worksheet.SalesOrderJobTask.Id) {
                ...
                //deleted worksheet lines <<
                if (activeWorksheet.DeletedWorksheetLines) {
                    for (var j = 0; j < activeWorksheet.DeletedWorksheetLines.length; j++) {
                        var deletedWorksheetLine = activeWorksheet.DeletedWorksheetLines[j];
                        if (deletedWorksheetLine.TM__c == tmId) {
                            activeWorksheet.DeletedWorksheetLines.splice(j, 1);
                            j--;
                        }
                    }
                }
    ...
    cancelDeselectTMCallback : function(component, event, worksheetIndex, salesOrderJobTaskId, tmId) {
        this.closeModal(component, event);
        var worksheets = component.get("v.worksheets");
        var worksheet = worksheets[worksheetIndex];
        for (var i = 0; i < worksheet.RelatedInfos.length; i++) {
            if (worksheet.SalesOrderJobTask.Id == salesOrderJobTaskId && worksheet.RelatedInfos[i].TM.Id == tmId) {
                worksheet.RelatedInfos[i].TM.Selected = true;
                break;
            }
        }
    ...
    setNextWorksheetLineNo : function(component, activeWorksheets) {
        ...
        for (var i = 0; i < activeWorksheets.length; i++) {
            ...
            for (var j = 0; j < activeWorksheet.WorksheetLines.length; j++) {
                var worksheetLine = activeWorksheet.WorksheetLines[j];
                if (nextWorksheetLineNo < worksheetLine.Line_No__c + 1) {
                    nextWorksheetLineNo = worksheetLine.Line_No__c + 1;
                }
            }
        }
    ...
    //TM lookup <<
    createTMLookupOptions : function(component, event) {
        var activeWorksheets = component.get("v.activeWorksheets");
        for (var i = 0; i < activeWorksheets.length; i++) {
            var activeWorksheet = activeWorksheets[i];
            var options = [];
            ...
            for (var j = 0; j < activeWorksheet.WorksheetLines.length; j++) {
                ...
                if (worksheetLine.TM__c) {
                    if (!tmIds.includes(worksheetLine.TM__c)) {
                        var option = {"Name": worksheetLine.TM__r.Name, "Id": worksheetLine.TM__c, "Scheduled_Date__c": worksheetLine.TM__r.Scheduled_Date__c, "TM_Job_Task__c": worksheetLine.TM_Job_Task__c, "TM_Job_Task__r": worksheetLine.TM_Job_Task__r };
                        options.push(option);
            ...
            //ticket 20170 <<
            /*
            activeWorksheet.TMNoOptions = options;
            var worksheetComponent = component.find("active-worksheet");
            if (worksheetComponent) {

                if (Array.isArray(worksheetComponent)) {
                    //ticket 19942 <<
                    //worksheetComponent[i].updateTMNoOptions();
                    for (var j = 0; j < worksheetComponent.length; j++) {
                        if (worksheetComponent[j].get("v.worksheetIndex") == i) {
                            worksheetComponent[j].updateTMNoOptions();
                            break;
                        }
                    }
                    //ticket 19942 >>
                    ...
                    worksheetComponent.updateTMNoOptions();
                }
            }*/
            component.set("v.activeWorksheets[" + i + "].TMNoOptions", options);
            //ticket 20170 >>
        }
    },
    //TM lookup >>
    //ticket 20170 <<
    sortActiveWorksheetTabs : function(worksheets) {
        this.sortWorksheetsByTaskNo(worksheets);
        var sorted = [];
        for (var i = 0; i < worksheets.length; i++) {
            sorted.push(worksheets[i]);
        }
        return sorted;
    },
    sortWorksheetsByTaskNo : function(worksheets) {
        worksheets.sort(function(a, b) {
            return sort(a, b);
        });

        function sort(a, b) {
            var val1 = parseInt(a['SalesOrderJobTask']['Task_No__c']);
            var val2 = parseInt(b['SalesOrderJobTask']['Task_No__c']);
            var order = 0;
            if (val1 > val2) {
                order = 1;
            } else if (val1 < val2) {
    ...
    getWorksheetIndex : function(worksheet, worksheets) {
        var worksheetIndex = 0;
        for (var i = 0; i < worksheets.length; i++) {
            if (worksheet.SalesOrderJobTask.Task_No__c == worksheets[i].SalesOrderJobTask.Task_No__c) {
                worksheetIndex = i;
                break;
            }
        }
