({
    initIterationTemplate : function(component, event) {
		...
		var serviceCenterAdvancedDisposal = component.get("v.advDisposalFlag");
        var columnHeaders = [];
        //columnHeaders.push({label: "", headerStyle: "min-width:35px;max-width:35px;text-align:center;"});
        console.log('serviceCenterAdvancedDisposal:::::::::::::'+serviceCenterAdvancedDisposal);
        if (fixedColumns) {
            columnHeaders.push({label: "", headerStyle: "min-width:35px;max-width:35px;text-align:center;position:sticky;left:0px;z-index:1000;"});
        }
        else {
            columnHeaders.push({label: "", headerStyle: "min-width:35px;max-width:35px;text-align:center;"});
        }
        ...
        //var template = component.get("v.template");
        var template = component.get("v.savedTemplate");
        if (template) {
            for (var i = 0; i < template.length; i++) {
                ...
                if (columnElement && columnElement.componentDef && columnElement.componentDef.descriptor == "markup://c:InlineEditColumnHeader") {
                    ...
                    if (columnElement.attributes.values.label) {
                        ...
                        if(columnHeader.label === 'To Invoice'){
                            console.log('columnHeader.label===>'+JSON.stringify(columnHeader.label));
                            var serviceCenterIntegration = component.get("v.serviceCenterIntegration");
                            
                            columnHeader.label = serviceCenterIntegration? 'To Billing Package': 'To Invoice';
                            console.log('columnHeader.label after ===>'+JSON.stringify(columnHeader.label));
                            //console.log('serviceCenterAdvancedDisposal Profile Approval*************'+serviceCenterAdvancedDisposal);
                        }
                    }
                    ...
                    console.log('before if serviceCenterAdvancedDisposal****************'+serviceCenterAdvancedDisposal);
                    if(serviceCenterAdvancedDisposal){
                    	columnHeaders.push(columnHeader);
                    }else{
                        if(columnElement.attributes.values.label.value != 'Profile Approval' && columnElement.attributes.values.label.value != 'Disposal Billing Method'){
                    	//console.log('inside if****************'+columnElement.attributes.values.label.value);
                        columnHeaders.push(columnHeader);
                    }
    ...
    filter : function(component) {
        ...
        for (var i = 0; i < items.length; i++) {
            var visible = columnHeaders.reduce(function(show, columnHeader) {
                if (columnHeader.filterField && columnHeader.action == 'filter') {
                    ...
                    if (columnHeader.filterFieldType == 'date') {
                        ...
                        if (columnHeader.filterValue == null || columnHeader.filterValue == '') {
                            show = show && columnValue == null;
                        }
                        else {
    ...
    buildTable : function(component, event) {
        ...
        this.initIterationTemplate(component, event);
        this.buildParentChildRelationshipMap(component);
    },
});